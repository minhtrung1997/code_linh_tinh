        ...
        for r in records:  # Each record (r) is a contig with a unique LOCUS name in the GenBank file.
            ...
            for f in r.features:  # Iterate through every feature of the current contig.
                ...
                if feature_type in features:
                    ...
                    if feature_type != "gene" and args.prot_seq:
                        if "Transferred_Translation" in f.qualifiers.keys():
                            line += [f.qualifiers["Transferred_Translation"][0]]
                        elif 'translation' in f.qualifiers.keys():
                            line += [f.qualifiers["translation"][0]]
                        elif len(str(f.extract(r.seq))) % 3 != 0:
                            logging.debug(f"WARNING: Length of {seq.id} --> {cid} is not a multiple of 3 (it is {len(feat.extract(seq).seq)})")
                        else:
                            line += [str(f.extract(r.seq).translate())]
